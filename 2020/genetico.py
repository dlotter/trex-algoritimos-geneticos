...
NUM_INDIVIDUOS = 25  # Tamanho da população
...
def proxima_geracao(populacao, fitness):
    ...
    populacao = ordenar_lista(populacao, fitness)
    for i in range(NUM_MELHORES):
        proxima_ger.append(populacao[i])
    ...
    while len(proxima_ger) < NUM_INDIVIDUOS:
        ...
        individuo1,individuo2 = random.choices(populacao, weights=None, k=2)
...
for ger in range(num_geracoes):
    ...
    for individuo in populacao:
        indFitness = calcular_fitness(jogo, individuo)
        fitness.append(indFitness)
